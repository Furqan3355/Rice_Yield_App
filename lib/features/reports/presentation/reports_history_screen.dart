...
class _ReportsHistoryScreenState extends ConsumerState<ReportsHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    // ✅ Auto-refresh reports whenever reportsProvider data changes
    // This listens to the stream and keeps UI in sync without manual refresh button
    ref.listen(reportsProvider, (previous, next) {
      // Optional: Show snackbar when new data arrives
      // if (previous != null && next != previous) {
      //   ScaffoldMessenger.of(context).showSnackBar(
      //     const SnackBar(content: Text('Reports updated'), duration: Duration(seconds: 1)),
      //   );
      // }
    });
