...
class _UploadScreenState extends ConsumerState<UploadScreen> {
  ...
  //     }
  //   } catch (e) {
  //     debugdebugPrint("Background Analysis Error: $e");
  //   }
  // }
...
//   try {
//     debugPrint("🚀 Sending binary video to model (Web/Mobile Compatible)...");
...
//     if (response.statusCode == 200) {
//       final data = jsonDecode(response.body);
//       debugPrint("✅ Model Data Received: ${data['stats']}");
...
Future<void> _markAsFailed(String id) async {
  try {
    await ref.read(supabaseProvider)
        ...
        .eq('id', id);
    debugPrint("⚠️ Report status updated to failed.");
  } catch (e) {
  ...
  try {
    debugPrint(" Uploading binary video: ${videoFile.name}");
    ...
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      debugPrint(" Model Data Received: $data");
      ...
      debugPrint(" Sab kuch sahi save ho gaya!");
    } else {
      debugPrint(" Server Error: ${response.body}");
      await _markAsFailed(reportId);
    }
